---
name: ui-components
description: UI 组件开发技能。在需要创建页面、组件、表单、表格、弹窗等 UI 元素时使用此技能。默认使用 HeroUI（如需 shadcn/ui 必须明确说明）。适用于 React + Tailwind CSS + 原子化设计模式。
---
...
```
src/components/
├── ui/           # Atoms - 基础组件 (HeroUI，若指定则用 shadcn/ui)
...
最基础的 UI 元素，优先使用 HeroUI；只有在明确要求时使用 shadcn/ui。
