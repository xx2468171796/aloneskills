---
name: ui-components
description: UI 组件开发技能。在需要创建页面、组件、表单、表格、弹窗等 UI 元素时使用此技能。默认使用 shadcn/ui。适用于 React + Tailwind CSS + 原子化设计模式。
---
...
> 版本号见 `@rules/versions.md` | 所需模块: core（始终激活）

- **React** - UI 框架
- **shadcn/ui** - 默认组件库（CLI v4 按需安装）
- **Tailwind CSS** - 样式引擎 (Oxide Engine)
- **Lucide React** - 图标库
...
最基础的 UI 元素，统一使用 shadcn/ui。通过 `pnpm dlx shadcn@latest add <component>` 按需安装。
...
- ❌ 手写原生 HTML 按钮/输入框（使用 shadcn/ui 组件）
- ❌ 创建 .css/.scss/.less 文件
- ❌ 使用 JS 判断窗口宽度
- ❌ 组件超过 150 行不拆分
- ❌ 在组件中直接写数据获取逻辑
